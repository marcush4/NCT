...
from notebooks.TsaoLabNotebooks.CodeAndFigsForPaper.region_select import *
...
def get_loadings_and_top_neurons(dimreduc_df, session_key, dim_select=6, n=10):
...
def subset_selection(dimreduc_df, quantile, session_key, region='M1', figpath='.'):
    if region in ['AM', 'ML']:
    ...
    path = '%s/subset_indices' % os.path.dirname(figpath)
...
def make_scatter(dimreduc_df, session_key, region, figpath='.'):
    if region in ['AM', 'ML']:  
        dimreduc_df_sess = apply_df_filters(dimreduc_df, **{'loader_args':{'region': region}})
        loadings_pca, loadings_fca = get_loadings_and_top_neurons(dimreduc_df_sess, session_key, dim_select=21, n=10)
    else:
        loadings_pca, loadings_fca = get_loadings_and_top_neurons(dimreduc_df, session_key, n=10)
    #figpath = '/home/akumar/nse/neural_control/figs/revisions/IS_scatter%s.pdf' % region
    ...
    fig_save_path = '%s/IS_scatter%s.pdf' % (figpath, region)
    fig.savefig(fig_save_path, bbox_inches='tight', pad_inches=0)

def run_lda_analysis(dimreduc_df, data_path, session_key, region='M1', zscore=False, overwrite=False):
    ...
    if not os.path.exists('tmp/' + savepath) or overwrite:
        ...
        for h, session in enumerate(sessions):
            if region in ['AM', 'ML']:  
                ...
                x = get_psth(data_path, region, session, zscore=zscore, full_arg_tuple=dimreduc_df_sess['full_arg_tuple'])
            else:
                x = get_psth(data_path, region, session, zscore=zscore)
            xall.append(x)
...
def plot_lda_analysis(dimreduc_df, session_key, region='M1', quantile=0.75, figpath='.'):
    ...
    #figpath = '/home/akumar/nse/neural_control/figs/revisions/umap_clusteringLDA%s.pdf' % region
    save_fig_path = '%s/umap_clusteringLDA_%s.pdf' % (figpath, region)
    fig.savefig(save_fig_path, bbox_inches='tight', pad_inches=1)
    ...
    #figpath = '/home/akumar/nse/neural_control/figs/revisions/%s_lda_viz.pdf' % region
    save_fig_path = '%s/%s_lda_viz.pdf' % (figpath, region)
...
if __name__ == '__main__':
    ...
    make_scatter(df, session_key, region, figpath=figpath) 
    for q in np.array([0.5, 0.6, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95]):
         subset_selection(df, q, session_key, region, figpath=figpath)
    run_lda_analysis(df, data_path, session_key, region, zscore=False, overwrite=True)
    plot_lda_analysis(df, session_key, region, figpath=figpath)
    

    """
    Need to manually override the unit selection in Alfie dataset, then see if things work.
    Also, make sure this runs on catscan with appropriate unit selection.
    """
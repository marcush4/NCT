...
def get_random_projections(region):
    ...
    if region in ['AM', 'ML']:
        ...
        random_proj_path = f'/clusterfs/NSDS_data/FCCA/postprocessed/tsao_jpca_{region}_randomcontrol.pkl'        
...
def Fig6Analyses(dimreduc_df, session_key, data_path, region, DIM, zscore=False, figpath='.'):
    ...
    ################## Perform jPCA on data
    resultsd3 = []
    data_files = np.unique(dimreduc_df['data_file'])
    jDIM = DIM - 1 if DIM % 2 != 0 else DIM    # jPCA dimension must be even

    for i, session in tqdm(enumerate(sessions)):

        if region in ['AM', 'ML']:
            df_ = apply_df_filters(dimreduc_df, data_file=session)
            y = get_spikes(data_path, region, session, zscore=zscore, full_arg_tuple=df_['full_arg_tuple'])
        else:
            y = get_spikes(data_path, region, session, zscore=zscore)

         # GET TRANSITION TIMES FOR NON-TRIALIZED DATA #
        if region in ['M1', 'S1']:
            dpath = '/mnt/Secondary/data/sabes'
            dat = load_sabes('%s/%s' % (dpath, session))
            dat = reach_segment_sabes(dat, data_file=session.split('.mat')[0])
            transition_times = dat['transition_times']

        for dimreduc_method in ['LQGCA', 'PCA']:

            if region in ['AM', 'ML']:
                df_ = apply_df_filters(dimreduc_df, data_file=session, fold_idx=0, dim=DIM, dimreduc_method=dimreduc_method, loader_args={'region': region})
                if dimreduc_method == 'LQGCA':
                    df_ = apply_df_filters(df_, dimreduc_args={})
            else:
                df_ = apply_df_filters(dimreduc_df, data_file=session, fold_idx=0, dim=DIM, dimreduc_method=dimreduc_method)
                if dimreduc_method == 'LQGCA':
            ...
            # Project data, using trialized structure if necessary
            #pdb.set_trace()
            yproj = y @ V
            if region not in ['AM', 'ML']:
                yproj = np.array([yproj[t0:t0+20] for t0, t1 in transition_times if t1 - t0 > 21])

            result_ = {}
            result_['data_file'] = session
    ...
    control_reps = np.unique(controldf['inner_rep']).size
    ...
    for i, session in enumerate(sessions):
        ...
        df_fcca = apply_df_filters(dimreduc_df, **fcca_filter)
        df_pca = apply_df_filters(dimreduc_df, **pca_filter)

        make_rot_plots(y, df_fcca, df_pca, maxim, region, jDIM, figpath)
        
        y = y[:, :12, :] if region in ['AM', 'ML'] else y
        make_traj_amplification_plots(df_fcca, y, DIM, region, figpath)
        make_traj_amplification_plots(df_pca, y, DIM, region, figpath)
...
if __name__ == '__main__':
    ...
    """"
        Inspect how I've gotten the transition_times above.
    """
    ...
    Fig6Analyses(df, session_key, data_path, region, DIM, zscore=False, figpath=figpath)

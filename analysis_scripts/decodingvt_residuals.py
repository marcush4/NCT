...
from decoders import lr_residual_decode_windowed
...
def gen_run(name, decorrelation='entire', didxs=np.arange(35), error_filt_params=[(1., 'le')], reach_filt_params=[(0, 0, 'le')]):
    combs = itertools.product(didxs, error_filt_params, reach_filt_params)
    with open(name, 'w') as rsh:
        rsh.write('#!/bin/bash\n')
        for (di, efp, rfp) in combs:
            rsh.write('mpirun -n 8 python decodingvt_residuals.py %s %d %d --error_thresh=%.2f --error_op=%s --q=%.2f --filter_op=%s\n' % 
                      (decorrelation, di, rfp[0], efp[0], efp[1], rfp[1], rfp[2]))
# Filter reaches by:
# 0: Nothing
...
if __name__ == '__main__':
    ...
    for j, window in enumerate(windows):
        for fold, (train_idxs, test_idxs) in tqdm(enumerate(KFold(n_splits=5).split(Z))): 
            ...
            r2pos, r2vel, r2acc, r2post, r2velt, r2acct, reg, ntr_, fitr, fite, msetr, msete  = lr_residual_decode_windowed(xpca, Z, lag, [window], [window], transition_times, train_idxs=tt_train_idxs,
                                                                                                                test_idxs=tt_test_idxs, decoding_window=decoding_window, decorrelation=decorrelation) 
            ...
            r2pos, r2vel, r2acc, r2post, r2velt, r2acct, reg, ntr_, fitr, fite, msetr, msete  = lr_residual_decode_windowed(xfcca, Z, lag, [window], [window], transition_times, train_idxs=tt_train_idxs,
                                                                                                                   test_idxs=tt_test_idxs, decoding_window=decoding_window, decorrelation=decorrelation)
    ...
    dpath = '/home/akumar/nse/neural_control/data/decodingvt_residual_%s' % decorrelation

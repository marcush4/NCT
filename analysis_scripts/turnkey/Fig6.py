...
def make_rot_plots(x, df_fcca, df_pca, maxim, region, jDIM, figpath):
    ...
    for i in range(0, 25):
        ...
        theta_idx = 11 #15
        theta0 = np.arctan2(trajectory[theta_idx, 1], trajectory[theta_idx, 0])
...
def make_box_plots(maxim, maxim_control, region, figpath):
    ...
    # Boxplots
    fig0, ax0 = plt.subplots(figsize=(4, 1.5))
    ...
    _, p1 = scipy.stats.wilcoxon(r1, r2, alternative='greater')
    _, p2 = scipy.stats.wilcoxon(r1, r2, alternative='less')
    print('Rotational test p: %f' % min(p1, p2))

    # test that each is stochastically greater than the median random
    _, p1 = scipy.stats.wilcoxon(r1, alternative='greater')
    _, p2 = scipy.stats.wilcoxon(r2, alternative='greater')
    ...
    ax0.set_xlabel('Rotational Strength above Random', fontsize=9)
    #ax.set_title('****', fontsize=14)

    ax0.invert_yaxis()
    ax0.set_xticks(rot_xticks[region])
    ax0.set_xlim(rot_xlims[region])
    ...
    bplot = ax1.boxplot([r3, r4], patch_artist=True, 
                    medianprops=medianprops, notch=False, vert=False, showfliers=False, widths=[0.3, 0.3],
                    whiskerprops=whiskerprops, showcaps=False)

    _, p1 = scipy.stats.wilcoxon(r3, alternative='greater')
    _, p2 = scipy.stats.wilcoxon(r4, alternative='greater')
    _, p3 = scipy.stats.wilcoxon(r3, r4, alternative='less')

    print('Amplification p test: %f' % p3)
    method1 = 'FBC'
    method2 = 'FFC'

    ax1.set_yticklabels([method1, method2], fontsize=12)
    ax1.set_xticks(dyn_xticks[region])
    ax1.set_xlim(dyn_xlims[region])
    ax1.tick_params(axis='both', labelsize=12)
    #ax.set_ylabel(r'$\sum_i Im(\lambda_i)$', fontsize=22)
    ax1.set_xlabel('Dynamic Range above Random', fontsize=9)
    #ax.set_title('****', fontsize=14)
    ...
    # Return dynamic ranges for across area comparison
    return r1, r2, r3, r4
...
def plotFig6(decoding_df, session_key, data_path, region, DIM, figpath='.'):
    ...
    for i in range(len(sessions)):
        # Theres an issue with overwriting the data_path in other sections (load_data in region_select). This overwrote the data_file. 
        # Delete manual_session when solved. Also a problem in loaders.py because it's looking for wrong data path.
        """MIH: Some issues with Tsao lab data. Need to do some reworking, see above."""
        
        #manual_session = A_df[A_df['data_file'].str.contains(sessions[i].split('_')[-1].split('.')[0])]['data_file'].iloc[0]
        
        for j, dimreduc_method in enumerate(['LQGCA', 'PCA']):
            #df_ = apply_df_filters(A_df, **{session_key: manual_session, 'dimreduc_method':dimreduc_method})
        ...
        for j in range(control_reps):
            #df_ = apply_df_filters(controldf, **{session_key: manual_session, 'inner_rep':j})
    ...
    rot_f, rot_p, dr_f, dr_p = make_box_plots(maxim, maxim_control, region, figpath)
    ...
    for i, session in enumerate(sessions):
        if region in ['AM', 'ML']:
            ...
            unique_loader_args = list({frozenset(d.items()) for d in decoding_df['loader_args']})
            loader_args=dict(unique_loader_args[loader_kwargs[region]['load_idx']])
            decoding_df = apply_df_filters(decoding_df, **{'loader_args':loader_args})
    ...
    '''
    return rot_f, rot_p, dr_f, dr_p

if __name__ == '__main__':

    #regions = ['M1', 'S1', 'M1_maze', 'HPC_peanut', 'AM', 'ML']
    #regions = ['HPC', 'mPFC']
    regions = ['VISp'] #['M1','S1', 'AM','ML','VISp', 'HPC_peanut']
    ...
    for region in tqdm(regions):
        ...
        rot_f, rot_p, dr_f, dr_p = plotFig6(df, session_key, data_path, region, DIM, figpath=figpath)

...
def make_scatter(dimreduc_df, figpath, subset_label):
...
def run_lda_analysis(dimreduc_df, data_path, ssidx, subset_files, M1=True, overwrite=False):

    if M1:
        savepath = 'psth_clustering_tmpM1_ss%d.pkl' % ssidx
        region = 'M1'
    else:
        savepath = 'psth_clustering_tmpS1_ss%d.pkl' % ssidx
        region='S1'
    if not os.path.exists('tmp/' + savepath) or overwrite:

        # Need to also load the subset indices and remove the discarded neurons
        with open('/home/akumar/nse/neural_control/subset_indices/%s' % subset_files[ssidx], 'rb') as f:
...
def plot_lda_analysis(dimreduc_df, ssidx, M1=True, quantile=0.75):
    ...
    # fig.tight_layout()
    if M1:
        fig.savefig('/home/akumar/nse/neural_control/figs/revisions/umap_clusteringLDAM1_ss%d.pdf' % ssidx, bbox_inches='tight', pad_inches=1)
    else:
        fig.savefig('/home/akumar/nse/neural_control/figs/revisions/umap_clusteringLDAS1_ss%d.pdf' % ssidx, bbox_inches='tight', pad_inches=1)
    ...
    if M1:
        fig.savefig('/home/akumar/nse/neural_control/figs/revisions/m1_lda_viz_ss%d.pdf' % ssidx, bbox_inches='tight', pad_inches=0)
    else:
        fig.savefig('/home/akumar/nse/neural_control/figs/revisions/s1_lda_viz_ss%d.pdf' % ssidx, bbox_inches='tight', pad_inches=0)
...
if __name__ == '__main__':
    ...
    subset_labels = ['rFFC excl.', 'rFBC excl.',
                    'LDA FBC excl.', 'LDA FFC excl.']

    for subset_index in tqdm([0, 1, 2, 3]):
        if M1:
            ...
            df_subset = df.iloc[subset_filter]
            make_scatter(df_subset, '/home/akumar/nse/neural_control/figs/revisions/IS_scatter_subset%d.pdf' % subset_index, subset_labels[subset_index]) 
            # run_lda_analysis(df_subset, '/mnt/Secondary/data/sabes', subset_index, subset_files, M1)
            # plot_lda_analysis(df_subset, subset_index, M1)
        else:
            subset_files = ['pca_subset_q75_S1.pkl', 'fca_subset_q75_S1.pkl', 
                            'fca_ldasubset_q75_S1.pkl', 'pca_ldasubset_q75_S1.pkl']
            subset_filter = [idx for idx in range(df.shape[0]) if df.iloc[idx]['loader_args']['subset_file'] == subset_files[subset_index]]
            df_subset = df.iloc[subset_filter]
            make_scatter(df_subset, '/home/akumar/nse/neural_control/figs/revisions/IS_scatterS1_subset%d.pdf' % subset_index, subset_labels[subset_index]) 
            # run_lda_analysis(df_subset, '/mnt/Secondary/data/sabes', subset_index, subset_files, M1)
            # plot_lda_analysis(df_subset, subset_index, M1)
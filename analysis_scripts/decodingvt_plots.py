...
def load_data_M1():
    ...
    fls = glob.glob('/home/akumar/nse/neural_control/data/decodingvt_cv_ttshift_lag2w30/*.dat')
    rl = []
    for fl in fls:
...
def plot_(dvt_df, vel_all, acc_all, metrics, ax, region):
    ...
    for i, metric in enumerate(metrics):
        ...
        atwins[i][1].plot(50 * np.arange(-5, 30), np.mean(accall, axis=0), linestyle='--', color='teal', linewidth=2, alpha=1.)
    ...
    def plot_delta_trace(r2f, r2p, ax, color, label):
        ...
        dr2 = np.array([np.sum(np.multiply(r1 - r2, ntr))/np.sum(ntr) for (r1, r2) in zip(r2f, r2p)])
        dr201 = dr2/norm_

        # How do we handle the std?
        dr2std = np.array([np.sqrt(np.var(r1 - r2) * np.sum(ntr**2)/np.sum(ntr)**2) for (r1, r2) in zip(r2f, r2p)])
...
if __name__ == '__main__':
    ...
    if main:
        ...
        fig.tight_layout()
        fig.savefig('/home/akumar/nse/neural_control/figs/loco_indy_merge/decodingvt_revised.pdf', bbox_inches='tight', pad_inches=0.1)

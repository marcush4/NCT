...
def gen_run(name, didxs, error_filt_params, reach_filt_params):
    combs = itertools.product(didxs, error_filt_params, reach_filt_params)
    with open(name, 'w') as rsh:
        rsh.write('#!/bin/bash\n')
        for (di, efp, rfp) in combs:
            rsh.write('mpirun -n 4 python decodingvt.py %d %d --error_thresh=%.2f --error_op=%s --q=%.2f --filter_op=%s\n'
...
if __name__ == '__main__':
    ...
    dpath = '/home/akumar/nse/neural_control/data/decodingvt3_copy'
